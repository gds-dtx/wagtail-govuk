...
| `dev.py` | **default** for `manage.py` locally; also the dev environment | SQLite (`db.sqlite3`) | `DEBUG=True`, dummy `SECRET_KEY="abc123"`, verbose logging on |
...
| `runtime.py` | picks dev vs deployed | — | `runserver` defaults to dev; gunicorn must set `DJANGO_SETTINGS_MODULE` explicitly |

`DJANGO_SETTINGS_MODULE` defaults to `govuk.settings.dev` for local workflows.
